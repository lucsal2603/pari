...
Le missioni della settimana escono da un serbatoio di 38 missioni (`MISSION_POOL` in `app.js`): ogni lunedì ne vengono estratte 7 a caso, con la stessa estrazione per tutti i telefoni della casa (seme = lunedì + codice casa) e al massimo 2 dello stesso tipo. Tutte si completano nell'istante in cui fai l'azione richiesta: nessuna aspetta la fine della settimana.
